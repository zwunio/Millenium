...
-> Patched for Mobile / Touch Support & UI Scaling
]]
-- Variables
...
if resizing and isMovement(input) then
local scaleObj = frame:FindFirstChildOfClass("UIScale")
local scaleFactor = scaleObj and scaleObj.Scale or 1
local delta = (input.Position - start) / scaleFactor

local viewport_x = camera.ViewportSize.X / scaleFactor
local viewport_y = camera.ViewportSize.Y / scaleFactor
...
library:connection(uis.InputChanged, function(input, game_event)
if dragging and isMovement(input) then
local scaleObj = frame:FindFirstChildOfClass("UIScale")
local scaleFactor = scaleObj and scaleObj.Scale or 1
local delta = (input.Position - start) / scaleFactor
...
Parent = coregui;
Name = "\0";
Enabled = true; -- Enabled for Toggle Button
...
library:create("UICorner", {Parent = toggleBtn, CornerRadius = dim(1, 0)})
library:create("UIStroke", {Parent = toggleBtn, Color = themes.preset.accent, Thickness = 2, ApplyStrokeMode = Enum.ApplyStrokeMode.Border})

local tDrag, tStart, tStartPos, tMoved = false, nil, nil, false
...
library:connection(uis.InputChanged, function(input)
    if tDrag and isMovement(input) then
        local delta = input.Position - tStart
        if delta.Magnitude > 5 then tMoved = true end
        local viewport = camera.ViewportSize
        local newX = math.clamp(tStartPos.X.Offset + delta.X, 0, viewport.X - 50)
        local newY = math.clamp(tStartPos.Y.Offset + delta.Y, 0, viewport.Y - 50)
        toggleBtn.Position = dim2(0, newX, 0, newY)
    end
end)
...
library:connection(uis.InputChanged, function(input)
if cfg.dragging and isMovement(input) then
...
local function bindDrag(obj, varName)
    obj.InputBegan:Connect(function(input)
        if isClick(input) then
            if varName == "sat" then dragging_sat = true
            elseif varName == "hue" then dragging_hue = true
            elseif varName == "alpha" then dragging_alpha = true end
            cfg.update_color()
        end
    end)
end
bindDrag(items["alpha_gradient"], "alpha")
bindDrag(items["hue_gradient"], "hue")
bindDrag(items["sat"], "sat")

uis.InputChanged:Connect(function(input)
if (dragging_sat or dragging_hue or dragging_alpha) and isMovement(input) then
cfg.update_color()
end
end)
library:connection(uis.InputEnded, function(input)
