...
-- NEW: Scaled Mouse Position Helper
...
function library:window(properties)
	local cfg = {
		...
		scale = properties.scale or properties.Scale or 1; -- ADDED SCALE
		selected_tab;
		items = {};
		tween;
	}
	
	library.scale = cfg.scale -- Set global scale
	...
	local items = cfg.items; do
		items["main"] = library:create("Frame", {
			...
			Size = cfg.size;
			Name = "\0";
			Position = dim2(0.5, -(cfg.size.X.Offset * cfg.scale) / 2, 0.5, -(cfg.size.Y.Offset * cfg.scale) / 2); -- FIXED CENTERING
...
function library:slider(options)
	...
	library:connection(uis.InputChanged, function(input)
		if cfg.dragging then
			local mousePos = library:getMousePos() -- FIXED FOR SCALE
